    ...
    return [
        ...
        },
        {
            ...
            "state": (
                ...
                if next_action in {"run_content_analysis", "wait_analysis_approval"}
                else "pending"
            ),
        },
        {
            ...
            "state": (
                "complete"
                if pedagogical_structure
                else "active"
                if next_action in {"run_pedagogical_design", "wait_structure_approval"}
                else "pending"
            ),
        },
        {
            ...
            "state": (
                ...
                if next_action in {"run_multimedia_generation"}
                else "pending"
            ),
        },
        {
            ...
            "state": (
                ...
                if next_action in {"export_pptx", "completed"}
                else "pending"
            ),
        },
    ]
...
def render_status_html(state: Dict[str, Any]) -> str:
    ...
    for idx, stage in enumerate(stages, start=1):
        ...
        stage_blocks.append(
            f"""
            <div class="progress-step {stage['state']}">
                <div class="step-dot">{idx}</div>
                <div class="step-label">{escape(stage['label'])}</div>
            </div>
            {connector}
            """.strip()
        )
    ...
    if missing_fields:
        badges = "".join(
            f'<span class="missing-chip">{escape(FIELD_LABELS.get(field, field))}</span>' for field in missing_fields
        )
